...
#     i,j,h,w = transforms.RandomCrop.get_params(img,output_size=(512,400))
...
def default_loader(img_path,mask_path,img_t_path,mask_t_path,mode):
    img_target = cv2.imread(img_t_path)
    mask_target = cv2.imread(mask_t_path,0)
    img_target = cv2.resize(img_target,(512,512))
    mask_target = cv2.resize(mask_target,(512,512))
    ...
    mask_target = mask_target.numpy() / 255 
    mask = mask.numpy()
    ...
    return img,mask,img_target,mask_target
...
def read_datasets(root_path,mode="train"):
    ...
    masks = []
    masks_names = []
    img_target, mask_target,_ = read_Heideberg(mode)
    ...
    if mode == "test":
        return images,masks,masks_names,img_target,mask_target

   
    return images,masks,masks_names,img_target * 2,mask_target * 2
...
class ImageFloder(data.Dataset):
    def __init__(self,root_path,datasets="CHOROID",mode="train"):
        ...
        if self.dataset == "CHOROID":
            self.images,self.labels,self.image_names,self.img_target,self.mask_target = read_datasets(self.root,self.mode)

    
    def __getitem__(self,index):
        img,mask,img_t,mask_t = default_loader(self.images[index],self.labels[index],self.img_target[index],self.mask_target[index],self.mode)
        img = torch.Tensor(img)
        mask = torch.Tensor(mask)
        img_t = torch.Tensor(img_t)
        mask_t = torch.Tensor(mask_t)
        if self.mode == "train" or self.mode == "test":
            return img,mask,img_t,mask_t,self.image_names[index]
        else:
...
class Choroid_loader:
    ...
    def load_test_data(self,batch_size):
        ...
        test_loader = data.DataLoader(dataset,batch_size,num_workers=0,shuffle=False,pin_memory=True)
        return test_loader
